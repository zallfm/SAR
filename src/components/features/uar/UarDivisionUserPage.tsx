...
const UarDivisionUserPage: React.FC<UarDivisionUserPageProps> = ({ onReview }) => {
    ...
    const overallProgress = useMemo(() => {
        ...
        const finishedCount = enhancedRecords.filter(record => record.status === 'Finished').length;
    ...
    const filteredRecords = useMemo(() => {
        ...
        return enhancedRecords.filter(record => {
            if (normalizedUarFilter && !record.searchableUarId.includes(normalizedUarFilter)) {
                return false;
            }

            if (normalizedOwnerFilter && !record.searchableOwner.includes(normalizedOwnerFilter)) {
                return false;
            }
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <tbody>
                            {paginatedRecords.map((record) => (
                                <tr key={record.id} className="bg-white border-b border-gray-200 last:border-b-0 hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-4 whitespace-nowrap text-sm"><StatusPill status={record.status} /></td>
                ...
                <div className="flex justify-between items-center mt-6 text-sm text-gray-500">
                    ...
                    <div className="flex items-center gap-4">
                        <span>Showing {totalItems === 0 ? 0 : `${startItem}-${endItem}`} of {totalItems}</span>
