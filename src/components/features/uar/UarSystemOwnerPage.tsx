...
const UarSystemOwnerPage: React.FC<UarSystemOwnerPageProps> = ({ onReview }) => {
    ...
    const filteredRecords = useMemo(() => {
        return records.filter(record => {
            const uarMatch = uarFilter ? record.uarId.toLowerCase().includes(uarFilter.toLowerCase()) : true;
            const ownerMatch = ownerFilter ? record.divisionOwner.toLowerCase().includes(ownerFilter.toLowerCase()) : true;
    ...
    }, [records, periodFilter, uarFilter, ownerFilter, statusFilter, progressData]);
                ...
                <div className="mb-6">
                    ...
                    <div className="flex justify-between items-start gap-4 mb-4 flex-wrap">
                        ...
                        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm w-full sm:w-auto sm:max-w-[240px]">
                            <p className="text-sm text-gray-500 font-medium">Progress Review</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        <div className="relative">
                            <input type="text" placeholder="UAR" value={uarFilter} onChange={e => setUarFilter(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
                        ...
                        <div className="relative">
                            <input type="text" placeholder="Division Owner" value={ownerFilter} onChange={e => setOwnerFilter(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><SearchIcon className="w-5 h-5 text-gray-400" /></div>
                        </div>
                        <div className="relative">
                            <input type={createDateFilter ? 'date' : 'text'} placeholder="Create Date" value={createDateFilter} onChange={e => setCreateDateFilter(e.target.value)} onFocus={e => { e.target.type = 'date'; }} onBlur={e => { if (!e.target.value) e.target.type = 'text'; }} className="w-full px-3 py-2 border border-gray-300 rounded-lg appearance-none bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
                        </div>
                        <div className="relative">
                        ...
                        <div className="relative">
                            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="w-full pl-3 pr-8 py-2 border border-gray-300 rounded-lg appearance-none bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">
                                <option value="">Status</option>
                                <option value="Finished">Finished</option>
                                <option value="InProgress">In Progress</option>
                            </select>
                            <div className="absolute inset-y-0 right-0 pr-2 flex items-center pointer-events-none"><ChevronDownIcon className="w-4 h-4 text-gray-400" /></div>
                        </div>
                    </div>
                </div>
                    ...
                    <table className="w-full min-w-[1000px] text-sm text-left text-gray-700">
                        <thead className="text-sm text-gray-800 font-bold bg-gray-50">
                            <tr className="border-b-2 border-gray-200">
                                {['UAR ID', 'Division Owner', 'Percent Complete', 'Create Date', 'Completed Date', 'Status', 'Action'].map(h =>
                                    <th key={h} scope="col" className="px-4 py-3 text-sm">{h}</th>
                                )}
                            </tr>
                        </thead>
                        <tbody>
                            {currentRecords.map((record) => {
                                ...
                                <tr key={record.id} className="bg-white border-b border-gray-200 last:border-b-0 hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-4 whitespace-nowrap text-sm">{record.completedDate}</td>
                                    <td className="px-4 py-4 whitespace-nowrap text-sm"><StatusPill status={status} /></td>
