...
const UarPicModal: React.FC<UarPicModalProps> = ({ onClose, onSave, picToEdit }) => {
    ...
    useEffect(() => {
        if (isEditMode && picToEdit) {
            setName(picToEdit.name);
            setDivision(picToEdit.division);
            setEmail(picToEdit.email);
            setDivisionQuery(picToEdit.division);
        }
    }, [picToEdit, isEditMode]);
    
    const filteredDivisions = useMemo(() => {
        if (!divisionQuery) return [...new Set(divisions)].sort();
        return [...new Set(divisions)].filter(d => 
    ...
    const handleSave = () => {
        ...
        const picData: PicUser = {
            id: isEditMode && picToEdit ? picToEdit.id : 0, // ID is handled by parent on creation
            name: name.trim(),
            division,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4" aria-modal="true" role="dialog">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
                <div className="flex justify-between items-center px-6 py-4 border-b">
                    ...
                    <button onClick={onClose} className="text-red-500 hover:text-red-700" aria-label="Close modal">
                        <CloseIcon />
                    </button>
                </div>
                <div className="p-6 space-y-4">
                    <div>
                        <label htmlFor="picName" className="block text-sm font-medium text-gray-700 mb-1">Name <span className="text-red-500">*</span></label>
                        <input
                            id="picName"
                            type="text"
                     ...
                     <div ref={divisionDropdownRef}>
                        <label htmlFor="picDivision" className="block text-sm font-medium text-gray-700 mb-1">Division <span className="text-red-500">*</span></label>
                        <div className="relative">
                            ...
                            {isDivisionDropdownOpen && (
                                <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-md shadow-lg mt-1 max-h-48 overflow-y-auto">
                                    {filteredDivisions.length > 0 ? filteredDivisions.map(d => (
                                        <li 
                                            key={d} 
                                        ...
                                        <li className="px-3 py-2 text-gray-500">No divisions found</li>
                                    )}
                                </ul>
                            )}
                        </div>
                    ...
                    <div>
                        <label htmlFor="picEmail" className="block text-sm font-medium text-gray-700 mb-1">Email <span className="text-red-500">*</span></label>
                        <div className="relative">
                            <input
                                id="picEmail"
                                type="email"
                ...
                <div className="flex justify-end items-center px-6 py-4 bg-gray-50 border-t gap-3 rounded-b-lg">
                     <button onClick={onClose} className="px-6 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none">
                        Cancel
                    </button>
                    <button
                        onClick={handleSave}
