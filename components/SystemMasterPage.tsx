...
const SystemMasterPage: React.FC<SystemMasterPageProps> = ({ user }) => {
    const [records, setRecords] = useState<SystemMasterRecord[]>(initialSystemMasterData);
    ...
    const [recordToDelete, setRecordToDelete] = useState<SystemMasterRecord | null>(null);

    const [infoMessage, setInfoMessage] = useState('');
    const [isInfoOpen, setIsInfoOpen] = useState(false);

    const systemTypes = useMemo(() => [...new Set(records.map(r => r.systemType))], [records]);

    const filteredRecords = useMemo(() => {
        return records.filter(record => {
            const typeMatch = systemTypeFilter ? record.systemType === systemTypeFilter : true;
            const codeMatch = systemCodeFilter ? record.systemCode.toLowerCase().includes(systemCodeFilter.toLowerCase()) : true;
    ...
    return (
        <div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full min-w-[1200px] text-sm text-left text-gray-600">
                        <thead className="text-sm font-bold text-gray-700 bg-gray-50">
                            <tr>
                                {['System Type', 'System Code', 'Valid From', 'Valid To', 'System Value Text', 'System Value Num', 'System Value Time', 'Created By', 'Created Date', 'Changed By', 'Changed Date', 'Action'].map(h => <th key={h} scope="col" className="px-4 py-3 border-b text-sm">{h}</th>)}
                            </tr>
                        </thead>
                        <tbody>
                            {currentRecords.map((record) => (
                                <tr key={record.id} className="bg-white border-b hover:bg-gray-50">
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.systemType}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.systemCode}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.validFrom}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.validTo}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.systemValueText}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.systemValueNum}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.systemValueTime}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.createdBy}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.createdDate.replace('\\n', ' ')}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.changedBy}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{record.changedDate.replace('\\n', ' ')}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="group relative">
                                                <button onClick={() => handleOpenDeleteConfirm(record)} className="text-gray-500 hover:text-red-600 text-sm" aria-label={`Delete ${record.systemCode}`}><DeleteIcon /></button>
                                                <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-max px-2 py-1 bg-blue-600 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
                                                    Delete
                                                </div>
                                            </div>
            ...
            {isModalOpen && <SystemMasterModal onClose={handleCloseModal} onSave={handleSaveRecord} recordToEdit={editingRecord} user={user} />}
            {isDeleteConfirmOpen && <ConfirmationModal onClose={handleCloseDeleteConfirm} onConfirm={handleDeleteRecord} title="Delete Confirmation" message="Do you want to Submit?" />}
            {isInfoOpen && <InfoModal onClose={() => setIsInfoOpen(false)} title="Information" message={infoMessage} />}
        </div>
    );
};
