...
const UarLatestRolePage: React.FC = () => {
    ...
    const [selectedRoleInfo, setSelectedRoleInfo] = useState<UarLatestRole | null>(null);

    const applicationIds = useMemo(() => [...new Set(roles.map(r => r.applicationId))], [roles]);
    const systemIds = useMemo(() => [...new Set(roles.map(r => r.roleId))], [roles]);

    const filteredRoles = useMemo(() => {
        return roles.filter(role => {
            const appIdMatch = appIdFilter ? role.applicationId === appIdFilter : true;
            const systemIdMatch = systemIdFilter ? role.roleId === systemIdFilter : true;
    ...
    const handleDownload = () => {
        ...
        const headers = ['Application ID', 'Username', 'NOREG', 'Name', 'Role ID', 'Role Description', 'Division', 'Departement', 'Position'];
                ...
                <div className="overflow-x-auto">
                    <table className="w-full min-w-[1400px] text-sm text-left text-gray-600">
                        <thead className="text-sm font-bold text-gray-700 bg-gray-50">
                            <tr>
                                {['Application ID', 'Username', 'NOREG', 'Name', 'Role ID', 'Role Description', 'Division', 'Departement', 'Position'].map(h => 
                                    <th key={h} scope="col" className="px-4 py-3 border-b text-sm">{h}</th>
                                )}
                            </tr>
                        </thead>
                        <tbody>
                            {currentRoles.map((role) => (
                                <tr key={role.id} className="bg-white border-b hover:bg-gray-50">
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{role.applicationId}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{role.username}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{role.noreg}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{role.name}</td>
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{role.roleDescription}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{role.division}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{role.department}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-sm">{role.position}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
